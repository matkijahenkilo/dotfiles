  ...
  home.packages =
    ...
    ++ [
      ...
      (pkgs.writeShellScriptBin "script-volume-decrease" ''
        ...
        ${lib.getExe pkgs.mpv} ${pkgs.sound-theme-freedesktop}/share/sounds/freedesktop/stereo/audio-volume-change.oga
      '')
      ...
      (pkgs.writeShellScriptBin "script-screenshot" ''
        ...
        if [[ $? != 0 ]]; then
          ${lib.getExe pkgs.mpv} ${pkgs.sound-theme-freedesktop}/share/sounds/freedesktop/stereo/dialog-error.oga
          exit 1
        fi
        wl-copy < $FILE_PATH
        notify-send 'Screenshot' -i "$FILE_PATH" "$FILE_NAME"
        ${lib.getExe pkgs.mpv} ${pkgs.sound-theme-freedesktop}/share/sounds/freedesktop/stereo/screen-capture.oga
      '')
